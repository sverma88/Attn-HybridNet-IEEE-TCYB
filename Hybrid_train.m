...
for stage = 1:HDNet.NumStages
    ...
    tic
    Tensor_Patches = create_tensor_patches_revised(OutImg, HDNet.PatchSize(stage));
    toc
    ...
    if stage ~= HDNet.NumStages % compute the PCA outputs only when it is NOT the last stage
        [OutImg_TD, ImgIdx_TD] = Tensor_output_revised(OutImg, ImgIdx, ...
...
save('HDNet_V.mat','V_TD','V_P','Tmean')
...
for idx = 1:NumImg
    ...
    [OutImg_i, ImgIdx_i] = Tensor_output_revised(OutImg_TD(OutImgIndex), ones(sum(OutImgIndex),1),...
    ...
    %%%%%% function to save training features
    if (0==mod(idx,5000) & idx~=0)
        name = ['Train_TDfeat_',num2str(idx),'.mat'];
        feat = cellfun(@convert_dense,f_TD(idx-4999:idx),'UniformOutput',false);
        feat = [feat{:}]';
        save(name,'feat');
        
        name = ['Train_PCAfeat_',num2str(idx),'.mat'];
        feat = cellfun(@convert_dense,f_P(idx-4999:idx),'UniformOutput',false);
        feat = [feat{:}]';
        save(name,'feat');      
